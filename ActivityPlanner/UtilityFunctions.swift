...
public func parseJsonFromURL<T: Decodable>(fullURL: String, as type: T.Type = T.self) -> T {
    let jsonFile = getJsonDataFromApi(apiUrl: fullURL)
    var arrayOfStructs: T?
    do {
        // Instantiate an object from the JSONDecoder class
        let decoder = JSONDecoder()
        ...
        // Use the decoder object to decode JSON objects into an array of structs of type T
        arrayOfStructs = try decoder.decode(T.self, from: jsonFile!)
    } catch {
        print("Unable to decode JSON file \(fullURL)!")
    }
